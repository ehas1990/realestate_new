...
class database {
	...
	*/
	function database( $host='localhost', $user, $pass, $db, $table_prefix = "" ) {
		...
		if (!function_exists( 'mysqli_connect' )) {
			die( 'FATAL ERROR: MySQL support not available.  Please check your configuration. Please Inform us if you see this Error,  E-mail : errorsql@imagehost.com   Thank You for Supporting US!' );
  			exit();
		}
		...
		die( '<br>MYSQL Error:'.mysqli_error().'<!--Please Inform us if you see this Error,  E-mail : errorsql@imagehost.com   Thank You for Supporting US!-->' );

            exit();
		}
		if (!mysqli_select_db($this -> _resource,$db)) {
			die( "Error: ".mysql_error().'<!--Please Inform us if you see this Error,  E-mail : errorsql@imagehost.com   Thank You for Supporting US!-->');
   			exit();
		}
	...
	function databaseReConnect(){
		if (!function_exists( 'mysql_connect' )) {
			die( 'FATAL ERROR: MySQL support not available.  Please check your configuration. Please Inform us if you see this Error,  E-mail : contact@kewlshare.com   Thank You for Supporting US!' );
			exit();
		}
		if (!($this->_resource = mysql_connect( $this -> _host, $this -> _user, $this -> _pass,true ))) {
		die( '<br>MYSQL Error:'.mysql_error().'<!--Please Inform us if you see this Error,  E-mail : contact@kewlshare.com   Thank You for Supporting US!-->' );

			exit();
		}
		if (!mysql_select_db($this -> _db)) {
			die( "Error: ".mysql_error().'<!--Please Inform us if you see this Error,  E-mail : contact@kewlshare.com   Thank You for Supporting US!-->');
			exit();
		}
	}
	...
	*/
	function getNumRows( $cur=null ) {
		...
		$rs= @mysqli_num_rows( $cur ? $cur : $this->_cursor );
		op($rs);
	...
	*/
	...
	/**
	 * @return The first element of the first row of the query
	 */
	
	function loadSingleColumn(){
		$cur= $this->_cursor;
		$ret = null;
