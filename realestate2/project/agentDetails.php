<?php
	...
	if(isset($_POST['act']) AND trim($_POST['act'])=="updateAgent"){
		...
		try{
			...
			$result = $property	->	updateMyAccount(trim($_GET['key']),trim($_POST['agentname']),trim($_POST['username']),trim($_POST['password']),trim($_POST['agentphone']),trim($_POST['district']),trim($_POST['panchayath']));
			
			$OldFileNames=  $property -> loadOldFileNameByUserKey(trim($_GET['key']));
			
			if($fileUploadStatus1 == true) {
				
					
					$recordKey	=	$property	->	loadAgentRecordKeyByUserKey(trim($_GET['key']));
					$photoKey	=	$property	->	loadAgentPhotoKeyByUserKey(trim($_GET['key']));
					$dr1		=	$target_dir.trim($_GET['key']);
					...
					$result1	=	$property ->	updateRecordTableWithImage(trim($_GET['key']),$_FILES['myphoto']['name'],$_FILES['myphoto']['type']);
				}
			...
			if($fileUploadStatus2 == true) {
					...
					$recordKey	=	$property	->	loadAgentRecordKeyByUserKey(trim($_GET['key']));
					$adharKey	=	$property	->	loadAgentAdharKeyByUserKey(trim($_GET['key']));
					...
					$result2	=	$property ->	updateRecordTableWithAadhar(trim($_GET['key']),$_FILES['aadhar']['name'],$_FILES['aadhar']['type']);
			}
	...
	$agentDetails 	=	$property	->	loadAgentDetailsByUserKey4(trim($_GET['key']));
