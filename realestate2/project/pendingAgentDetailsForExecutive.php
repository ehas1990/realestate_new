<?php
	...
	if(isset($_POST['act']) AND trim($_POST['act'])=="updateAgent"){
		...
		try{
			...
			$result = $property	->	updatePendingAgentAccount(trim($_GET['key']),trim($_POST['agentname']),trim($_POST['username']),trim($_POST['password']),trim($_POST['agentphone']),trim($_POST['district']),trim($_POST['panchayath']));
	...
	$agentDetails 	=	$property	->	loadPendingAgentDetailsByKey(trim($_GET['key']));
	...
	$smarty->display("pendingAgentDetailsForExecutive.tpl");
