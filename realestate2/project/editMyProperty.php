<?php
	...
	if(isset($_POST['act']) AND trim($_POST['act'])=="addPost"){
		...
		//	op($_POST);
			...
			$imageKeys	 = $property	->	loadPropertyImageKeysByPropertyKey($result['propertyKey']);
		
			
			if( $fileUploadStatus1 == true AND isset ($imageKeys[0]["img_key"])) {
			...
			if( $fileUploadStatus2 == true AND isset ($imageKeys[1]["img_key"])) {
			...
			if( $fileUploadStatus3 == true AND isset( $imageKeys[2]["img_key"])) {
			...
			if( $fileUploadStatus4 == true) {
				...
				if ( isset( $imageKeys[3]["img_key"])) {
					...
					$result4 = $property	-> updateEditImageTable($result['propertyKey'],$imageKeys[3]['img_key'],$_FILES['photo4']['name'],$_FILES['photo4']['type'],'4');	
						...
						$result1 = $property	-> updateImageTable($imgKey4,$result['userId'],$result['propertyId'],$_FILES['photo4']['name'],$_FILES['photo4']['type'],$result['propertyKey'],'1','0');	
	...
	$smarty->display("editMyProperty.tpl");
